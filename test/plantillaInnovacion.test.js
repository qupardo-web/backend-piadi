...
test('Plantilla Innovación - Verificaciones de Base de Datos y Mapeo', async (t) => {
  ...
  await t.test('El rol "Innovación" existe en la tabla roles', async () => {
    const role = await Role.findOne({
      where: {
        name: 'Innovación'
      }
    });
    assert.ok(role, 'Debe existir el rol Innovación');
  ...
  await t.test('La plantilla "Innovación" contiene los 40 campos configurados en 3 hojas', async () => {
    ...
    assert.strictEqual(campos.length, 40, 'Debe tener exactamente 40 campos');
    ...
    assert.strictEqual(camposProyectos.length, 21, 'Hoja Proyectos debe mapear 21 campos a tabla Proyecto');
    ...
    camposSecciones.forEach(c => {
      assert.strictEqual(c.orden_insercion, 3, `Campo ${c.nombre_campo} debe tener orden_insercion = 3`);
  ...
  await t.test('La plantilla "Innovación" tiene archivoData binario Excel válido con las 3 hojas', async () => {
